...
Actions OIDC deploy from **`hochgi/conquarrow`**; Namecheap CNAME
checklist for `api.games.hochgi.com` and `ws.games.hochgi.com`.

API Gateway **base-path mapping** `/conquarrow` on both custom domains so
...
- Health (or empty) HTTP route under `/conquarrow` on the custom domain
- Lambdas: HTTP + WS, **60s timeout / 1024 MB** on the move function (P18 burst)
- WS connect URL is `wss://ws.games.hochgi.com/conquarrow`
- Bucket is private; Lambdas can read/write the `conquarrow/` prefix
