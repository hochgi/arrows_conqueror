...
| P37 | Immediate loss | rules | §9, §11 44 | P36 | **[packet](./packets/P37-immediate-loss.md).** Playtest: encircling the last enemy territory did not end the match — the win landed 4 moves later at the round boundary, with a dead seat moving in between. Resolve losses after every applied move. Resolves §11 item 44 by dissolution (no path can un-own a share) and pins the chain as an invariant |
