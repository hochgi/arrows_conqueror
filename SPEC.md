...
44. **What represents a match that ends with no surviving seat?** `GameState.winner` is `PlayerId | undefined`, and `undefined` already means *still playing* — `victoryFx` reads it that way (`packages/web/src/fx/victory.ts`). P36 made losing a per-seat event resolved at a round boundary, and two seats can reach *T>0, S=0, H=0* on the same boundary, so a board with **no** seat left is constructible. P36 deliberately did not invent a draw: it removes every qualifying seat, leaves `winner` unset, and the state is terminal-but-unwon. **The cost is worse than a misread banner.** With no winner set and no seat holding a legal step, `App.tsx`'s `passIfExhausted` fires, commits, re-runs its effect, and never latches — the soft-lock key only compares against the immediately previous key, and `activePlayer` advances on every pass. So the board does not merely *look* like it is still playing; it spins, auto-passing without bound. That is recorded as wrong rather than patched, because the fix is a rule decision. Deciding the representation — a third outcome value, a nominated survivor, or a rule that the last seat cannot be removed — is a game decision. → **§9**, → **P36**.
