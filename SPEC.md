...
>
> **§11's two remaining open rules questions are items 45 and 46** (P36: whether a lost seat's trail evaporates rather than clearing silently; P37: whether `legalMoves` and `apply` must refuse once `winner` is set). Item 44 was **resolved by dissolution** by P37 — a match with no surviving seat cannot be reached, because no path can un-own a spawner share and a share owner is never lost. Beside those, what remains is a **parked tuning table**: item 11's *R*, the band radii, force per band, and item 32's *N* are numbers only playtesting can set, and P09 owns setting them. Item **39** parks a territory-combat idea without blocking. None of the tuning items changes a rule.
>
...
46. **Once `winner` is set, must `legalMoves` and `apply` refuse?** They do not today. `legalMoves` never consults `winner`, so a seat that wins mid-turn keeps whatever allowance it had left and `apply` accepts those steps — the match is decided and the board still moves. Under P36 this window sat *between* turns, where only the adapter could see it; P37 opens it *inside* a turn for the first time, which is what surfaced the question. No adapter reaches it: `packages/web/src/App.tsx` sets `inputLocked` on `winner !== undefined`, and the bot policies break out of their loops on the same test. So this is a **totality gap in the core, not a live defect** — but a rules engine whose `apply` accepts moves in a finished match is one adapter away from being a real one, and "the UI stops you" is not a rule. The two candidate answers are not equivalent: refusing makes `winner` a terminal state and every post-win move a `ContractViolation`, which a replay of a truncated log would then throw on; offering nothing but the pass keeps `apply` total and lets a log run past the win harmlessly. → **§9**, → **P37**.
