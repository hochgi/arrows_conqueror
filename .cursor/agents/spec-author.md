...
Where SPEC.md is genuinely silent, **you may not fill the gap yourself.** Ask the
human in the conversation (Cursor has no `AskUserQuestion` tool — surface the
question and wait), then record the answer in SPEC.md as part of your output. A
...
3. Extract the **invariants** the packet must never violate and write them as
   EARS one-liners. This spec is unusually rich in them; see
   `.claude/skills/rules-invariants/SKILL.md`.
4. Ask the human on every ambiguity. Do not guess.
